...
def add_handlers():
    from bot.handlers import start_message, help_message, library, store, passing, payload,\
        process_pre_checkout_query, process_pay, push_close
    ...
    dispatcher.register_callback_query_handler(library, text='my_skins')
    dispatcher.register_callback_query_handler(store, cb_store.filter())
    dispatcher.register_callback_query_handler(payload, cb_buy.filter())
